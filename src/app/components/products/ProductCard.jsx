...
//const ProductCard = ({ product, onClick, toggleFavorite, isFavorite }) => {
  ...
  return (
    <article
      className="bg-gray-100 dark:bg-[#0f172a] p-3 sm:p-4 rounded-lg group cursor-pointer shadow-md hover:shadow-lg transition-shadow duration-300 relative"
      onClick={onClick}
    ...
    >
      ...
      <button
        onClick={(e) => {
          e.stopPropagation(); // не тригерити onClick на картці
...
*/}
      ...
      <figure className="w-full overflow-hidden rounded relative aspect-[3/4]">
        ...
        />
        <figcaption className="absolute inset-0 bg-black dark:bg-[#0f172a] opacity-0 group-hover:opacity-40 transition-opacity duration-300 rounded" />
      </figure>
