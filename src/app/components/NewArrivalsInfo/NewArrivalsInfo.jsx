    ...
    <section className="bg-white dark:bg-zinc-900 py-14 px-5 transition-colors duration-300">
      ...
      <div className="grid gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
        {products.map((product) => {
          ...
          const productUrl = `/product/${product.id}`; // підстав свій реальний шлях
          ...
          return (
            ...
            >
              {/* schema.org: обов’язкові/корисні мета-поля */}
              ...
              <section className="mt-3 sm:mt-4">
                ...
                <p className="text-xs sm:text-sm text-gray-700 dark:text-gray-400 mt-1 text-center sm:text-left">
                  <span className="font-semibold">{priceLabel}:</span> {product.price}{' '}
                  <span className="text-xs">UAH</span>
                </p>
              </section>

              {/* schema.org: Offer (мінімально необхідно для Product) */}
      ...
      {/* Toast повідомлення */}
      {showToast && lastProduct && <Toast product={lastProduct} onClose={() => setShowToast(false)} />}
    </section>
  );
};
